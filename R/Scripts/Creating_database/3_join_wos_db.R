...
bodysize_joined <- bind_rows(db_raw, wos_raw) %>% 
  ...
  mutate(
    # Make uid column to give each data.point a uid aswell as each individual a uid
    uid = row_number(),
    
    # set original.source.codes to no.source for NAs - this isn;t for if there isn;t a source for the location it means there isn't that mnay locations fir this source so it would be an NA for this column but changing it to a NA code instead to avoid confusion what left joining later as some of the join,location codes are "NA" for a country
